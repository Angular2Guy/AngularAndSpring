...
public class BitstampService {
	...
	public Mono<byte[]> pdfReport(String timeFrame, String pair) {
		if (MongoUtils.TimeFrame.TODAY.getValue().equals(timeFrame)) {
			Query query = MongoUtils.buildTodayQuery(Optional.of(pair));
			return this.reportGenerator.generateReport(this.operations.find(query, QuoteBs.class)
					.filter(this::filter10Minutes).map(this.reportMapper::convert));
		} else if (MongoUtils.TimeFrame.SEVENDAYS.getValue().equals(timeFrame)) {
			Query query = MongoUtils.build7DayQuery(Optional.of(pair));
			return this.reportGenerator.generateReport(this.operations
					.find(query, QuoteBs.class, BS_HOUR_COL).map(this.reportMapper::convert));
		} else if (MongoUtils.TimeFrame.THIRTYDAYS.getValue().equals(timeFrame)) {
			Query query = MongoUtils.build30DayQuery(Optional.of(pair));
			return this.reportGenerator.generateReport(
					this.operations.find(query, QuoteBs.class, BS_DAY_COL).map(this.reportMapper::convert));
		} else if (MongoUtils.TimeFrame.NINTYDAYS.getValue().equals(timeFrame)) {
			Query query = MongoUtils.build90DayQuery(Optional.of(pair));
			return this.reportGenerator.generateReport(
					this.operations.find(query, QuoteBs.class, BS_DAY_COL).map(this.reportMapper::convert));
	...
	public void createBsHourlyAvg() {
		Tuple<Calendar, Calendar> timeFrame = ServiceUtils.createTimeFrame(this.operations, BS_HOUR_COL, QuoteBs.class, true);
	...
	public void createBsDailyAvg() {
		Tuple<Calendar, Calendar> timeFrame = ServiceUtils.createTimeFrame(this.operations, BS_DAY_COL, QuoteBs.class, false);
	...
	private QuoteBs avgBsQuote(QuoteBs q1, QuoteBs q2, long count) {
		QuoteBs myQuote = new QuoteBs(ServiceUtils.avgHourValue(q1.getHigh(), q2.getHigh(), count),
				ServiceUtils.avgHourValue(q1.getLast(), q2.getLast(), count), q1.getTimestamp(),
				ServiceUtils.avgHourValue(q1.getBid(), q2.getBid(), count), ServiceUtils.avgHourValue(q1.getVwap(), q2.getVwap(), count),
				ServiceUtils.avgHourValue(q1.getVolume(), q2.getVolume(), count), ServiceUtils.avgHourValue(q1.getLow(), q2.getLow(), count),
				ServiceUtils.avgHourValue(q1.getAsk(), q2.getAsk(), count), ServiceUtils.avgHourValue(q1.getOpen(), q2.getOpen(), count));
