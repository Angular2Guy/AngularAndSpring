...
public class PrepareData {
	...
	public void createHourlyAvg() {
		...
		while (begin.after(globalBeginn)) {
			...
			log.info("Prepared Data for: " + sdf.format(begin.getTime()));
		}
	}
		...
		try {
			...
			IntStream.range(0, QuoteCb.class.getConstructor(types).getParameterAnnotations().length)// .parallel()
					.forEach(x -> {
						...
						try {
							annotation = (JsonProperty) QuoteCb.class.getConstructor(types)
									.getParameterAnnotations()[x][0];
							String fieldName = annotation.value();
							Method[] methods = QuoteCb.class.getMethods();
							for (int y = 0; y < methods.length; y++) {
								Method method = methods[y];
								if (method.getName().matches("(?i:.*get" + fieldName + ".*)")) {
	...
	private BigDecimal avgHourValue(BigDecimal v1, BigDecimal v2, long count) {
		return v1.add(v2 == null ? BigDecimal.ZERO
				: v2.divide(BigDecimal.valueOf(count == 0 ? 1 : count), 2, RoundingMode.HALF_UP));
	}
}
