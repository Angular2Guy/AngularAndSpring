...
public class ScheduledTask {
	...
	public ScheduledTask(BitstampService bitstampService, MyUserService myUserService,
			BitfinexService bitfinexService, CoinbaseService coinbaseService, RestClient.Builder restClientBuilder) {
...
//	}
	...
	private void insertBsQuote(String currPair) {
		...
		try {
			...
			quote = this.limitPrecision(quote);
			this.bitstampService.insertQuote(quote);
			this.logDuration("Bitstamp", currPair, start);
		} catch (Exception e) {
			LOG.warn(String.format("Bitstamp data import failed for: %s", currPair), e);
		}
	}
	...
	public void insertCoinbaseQuote() {
		...
		try {
			...
			quote = this.limitPrecision(quote);
			this.coinbaseService.insertQuote(quote);
			this.logDuration("Coinbase", currPair, start);
		} catch (Exception e) {
			LOG.warn("Coinbase data import failed.", e);
		}
	}
	...
	private void insertBfQuote(String currPair) {
		...
		try {
			...
			this.bitfinexService.insertQuote(result);
			this.logDuration("Bitfinex", currPair, start);
		} catch (Exception e) {
			LOG.warn(String.format("Bitfinex data import failed for: %s", currPair), e);
		}
	}
